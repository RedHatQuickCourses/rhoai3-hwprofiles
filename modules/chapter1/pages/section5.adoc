...
[cite_start]Time-slicing allows multiple pods to share a single GPU by taking turns (interleaving) on the compute engine[cite: 3084]. This is ideal for interactive workbenches where users spend most of their time coding and only bursts of time executing cells.
...
. [cite_start]*Enable Time-Slicing:* Create a `ConfigMap` in the GPU Operator namespace defining the replicas (e.g., 4 slices per GPU)[cite: 3107].
. [cite_start]*Update Cluster Policy:* Configure the GPU Operator to read this map[cite: 3136].
...
[IMPORTANT]
====
With Time-Slicing, memory is shared. If one user consumes all VRAM, other users on the same GPU will crash (OOM). [cite_start]Always set strict `resourceLimits` in your profile to mitigate this risk[cite: 3090].
====
