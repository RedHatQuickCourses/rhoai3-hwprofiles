= Day 2 Operations: Governance, Scoping, and Troubleshooting
...
[TIP]
====
[cite_start]To enable the UI to distinguish between these scopes, ensure the dashboard configuration setting `disableProjectScoped` is set to `false`. [cite: 1915]
====
...
* *Fix:* Edit the Hardware Profile YAML. [cite_start]Add the exact `key`, `value`, and `effect` that matches the Node Taint. [cite: 55]
...
* *Check 3:* Check if the `identifiers` in the profile match what the Node Feature Discovery (NFD) operator is reporting. [cite_start]If NFD detects `nvidia.com/gpu` but your profile requests `nvidia.com/mig-1g.5gb`, the system may filter it out if no nodes report that specific MIG slice. [cite: 12-13]
...
* *Verify Queue:* Look for the label `kueue.x-k8s.io/queue-name`. [cite_start]Is it pointing to the correct Local Queue? [cite: 27]
